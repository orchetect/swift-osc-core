...
For the TCP protocol, both ``OSCTCPClientProtocol`` and ``OSCTCPServerProtocol`` are capable of sending messages using slightly different API.

### TCP Client

Ensure the client is connected to the remote server first (see ``OSCTCPClientProtocol`` docs), then you can begin sending data.
...
Ensure the server is started first (see ``OSCTCPServerProtocol`` docs), then you can begin sending data to connected clients after they have established a connection.

```swift
// send an OSC bundle or message to all currently connected clients
server.send(/* ... */)
    
// optionally, any errors may be handled by supplying a handler closure:
server.send(/* ... */) { clientID, error in
    // ...
}
```
...
let clientIDs = server.clients
    .filter { $0.value.host == "192.168.1.20" } // find the client(s) you want
    .keys // returns array of client IDs

// send, silently ignoring errors that may arise
server.send(/* ... */, toClientIDs: clientIDs)

// send, handling errors that may arise
server.send(/* ... */, toClientIDs: clientIDs) { clientID, error in
    // ...
}
```
